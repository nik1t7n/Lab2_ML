...
import { Package, Loader2, ChevronLeft, ChevronRight, ShoppingBasket } from 'lucide-react';
  ...
  const fetchProducts = async (page: number) => {
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/products/all?limit=${limit}&offset=${offset}`
      );
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex-grow space-y-4">
          <div className="flex items-center space-x-3">
            <ShoppingBasket className="w-5 h-5 text-gray-400 flex-shrink-0" />
            <p className="text-gray-600">
              Количество: <span className="font-semibold">{product.quantity}</span>
            </p>
          </div>
        </div>
      </div>
  ...
  <div className="container mx-auto px-4 py-6">
    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Наши продукты</h1>
          <p className="text-gray-600">
            {data ? `Всего продуктов: ${data.total_count}` : 'Загрузка...'}
          </p>
        </div>
      </div>
      <Link 
